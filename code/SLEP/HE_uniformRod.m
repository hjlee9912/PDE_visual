...
%with IC u(x,0) = f(x);
%add_my_path;
source = 0; %   0 = no source     1 = put source
...
c = @(x) x*0+6; %specific heat
rho = @(x) x*0+2.5; %mass density
K0 = @(x) x*0+3; %thermal conductivity
dK0dx = @(x) x*0;
...
xlabel('x')
ylabel('u')
title('Heat Flow in a Uniform Rod without Sources')
...
if source ==0
    filename = 'uniformRod_noSource.gif';
else
    filename = 'nonuniformRod_Source_jump.gif';
end
